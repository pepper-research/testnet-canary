...
use crate::IsInitialized;
use crate::{addresses::TrgId, Fractional, MPGId, ProductId, Side, ZERO_FRAC};

use crate::{
    dex::constants::MAX_OUTRIGHTS, dex::constants::MAX_TRADER_POSITIONS, AccountTag, DexError,
    DexResult, OpenOrders,
};
...
#[cfg_attr(
    ...
    schemars(bound = "S: ::sov_modules_api::Spec", rename = "TraderRiskGroup")
)]
...
pub struct TraderRiskGroup<S: Spec> {
    ...
    #[cfg_attr(
        ...
        schemars(with = "u8", length(equal = "MAX_OUTRIGHTS"))
    )]
    ...
    #[cfg_attr(
        ...
        serde(deserialize_with = "serde_arrays::deserialize"),
        serde(serialize_with = "serde_arrays::serialize"),
        schemars(with = "u8", length(equal = "256"))
    )]
...
impl<S: Spec> TraderRiskGroup<S> {
    ...
    pub fn is_active_product(&self, index: usize) -> Result<bool, DexError> {
    ...
    pub fn get_position_index(&self, index: usize) -> Result<u8, DexError> {
        if !self.is_initialized() {
            // msg!("TraderRiskGroup is not initialized");
            return Err(DexError::InvalidAccountData.into());
        }
