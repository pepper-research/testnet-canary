...
// use spicenet_risk::RiskModule;RiskModule
use sov_modules_api::Spec;

use crate::risk::risk_output::RiskEngineOutput;
use crate::{Fractional, MPGId, ProductId};

use crate::dex::{
    ComboProduct, DexError, DexResult, MpgAuthority, OutrightProduct, PriceEwma, Product,
    ProductStatus, ProductsArray, NAME_LEN,
};
use crate::BitPair;
...
#[cfg_attr(
    ...
    derive(schemars::JsonSchema),
    derive(sov_modules_api::macros::UniversalWallet),
    schemars(bound = "S: ::sov_modules_api::Spec", rename = "MarketProductGroup")
)]
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Debug, PartialEq, Clone, Eq)]
pub struct MarketProductGroup<S: Spec> {
    ...
    /// MPG Type
    pub mpg_type: MPGType,

    pub mpg_authority: MpgAuthority<S>,
    ...
    pub risk_output_register: RiskEngineOutput, // TODO: make type (struct)
    // module ids for risk and fee modules.
    pub risk_engine_module_id: ModuleId, // TODO: fill in type from module
    pub fee_model_module_id: ModuleId,   // TODO: fill in type from module
}

// impl<S: Spec> IsInitialized for MarketProductGroup<S> { // TODO: where is IsInitialized defined?
...
impl<S: Spec> MarketProductGroup<S> {
    ...
    pub fn find_outright(&self, product_key: &ProductId) -> DexResult<(usize, &OutrightProduct)> {
        let (idx, product) = self.find_product_index(product_key).unwrap();
    ...
    pub fn find_outright_among_all(
        &self,
        product_key: &ProductId,
    ) -> DexResult<(usize, &OutrightProduct)> {
    ...
    pub fn find_combo(&self, product_key: &ProductId) -> DexResult<(usize, &ComboProduct)> {
        let (idx, product) = self.find_product_index(product_key).unwrap();
    ...
    pub fn deactivate_product(&mut self, product_key: ProductId) -> DexResult {
    ...
    /// and assert if the product is expired using its product status. If not, we return a `DexError`, otherwise, we set the default value to the product at that specific `idx`
    pub fn remove_product(&mut self, product_key: &ProductId) -> DexResult {
        let (idx, _) = self.find_product_index_among_all(&product_key)?;
        if let Ok(outright) = self.active_products[idx].try_to_outright_mut() {
            assert_eq!(outright.is_expired(), true);
            // do we need this?
            if !outright.is_expired() {
                return Err(DexError::ContractIsNotExpired.into());
            }
        }
    ...
    pub fn add_product(&mut self, product: Product) -> DexResult {
